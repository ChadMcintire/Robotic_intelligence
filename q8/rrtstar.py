"""
Modified version from the following code:
https://github.com/zhm-real/PathPlanning/tree/master/Sampling_based_Planning/rrt_2D
It was a more versatile implementation for our grid environment then the one in the original repo
env.py, utils.py, myQueue.py, and plotting.py were the needed related files from the repo
    ...
    def planning(self):
        ...
        for k in range(self.iter_max):
            ...
            if node_new and not self.utils.is_collision(node_near, node_new):
                ...
                if distance.findPointDistance(node_new.x, node_new.y, self.s_goal.x, self.s_goal.y) < 1:
                    break

        index = self.search_goal_parent()
        self.path = self.extract_path(self.vertex[index])
        if show_animation:
            self.plotting.animation(self.vertex, self.path, "rrt*, N = " + str(self.iter_max), animation=True, show=False)
...
def main():
    ...
    def plan():
        rrt_star = RrtStar(x_start, x_goal, 10, 0.10, 20, 20000)
        rrt_star.planning()
        rx, ry = zip(*rrt_star.path)
        print(f"Distance Traveled: {distance.findPathDistance(rx, ry)}")
        print(f"Nodes visited: {len(rrt_star.vertex)}")
